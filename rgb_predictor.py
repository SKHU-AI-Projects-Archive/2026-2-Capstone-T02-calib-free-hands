...
# ---------------------------------------------------------------------------
# Resolve paths relative to this service root (MimicForge 이식: AnyHand 원본은
# scripts/ 하위에 있었으나 여기서는 서비스 루트에 flat 배치 — 가중치는 models/).
# ---------------------------------------------------------------------------
_THIS_DIR  = Path(__file__).resolve().parent          # services/hand-extractor/
...
class AnyHandPredictor:
    ...
    # ------------------------------------------------------------------
    _DEFAULT_WILOR_CKPT  = str(_REPO_ROOT / 'models' / 'anyhand_wilor.ckpt')
    _DEFAULT_WILOR_CFG   = str(_REPO_ROOT / 'models' / 'model_config_wilor.yaml')
    _DEFAULT_HAMER_CKPT  = str(_REPO_ROOT / 'models' / 'hamer_ckpts' / 'checkpoints' / 'anyhand_hamer.ckpt')
    _DEFAULT_DETECTOR_PT = str(_REPO_ROOT / 'models' / 'detector.pt')
    ...
    def _load_wilor(self) -> None:
        ...
        # [MimicForge] init_renderer=False — 키포인트 추출 전용이라 pyrender 불필요
        # (Windows 는 offscreen 렌더 플랫폼(egl/osmesa) 미지원이라 생성 시 죽는다)
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                for i in range(B):
                    ...
                    # [MimicForge] 왼손은 크롭이 좌우반전된 채 모델에 들어가므로
                    # (ViTDetDataset flip = right==0) 출력 x 를 되돌려야 원본 픽셀과 맞는다.
                    kp2d_norm = pred_kp2d[i].cpu().numpy().copy()
                    if is_right_i == 0.0:
                        kp2d_norm[:, 0] = -kp2d_norm[:, 0]
    ...
    def _predict_single(
        ...
        boxes, is_right, scores = self.detect_hands(img_bgr)
        if len(boxes) == 0:
            return []

        return self.reconstruct(img_bgr, boxes, is_right, scores, backend=backend)

    # ==================================================================
    # Public API — 단계별 호출 (검출 / 복원 을 따로 쓰고 싶을 때)
    # ==================================================================
    # [MimicForge] predict() 는 아래 두 단계를 이어 붙인 것. 외부(데모·실험)에서
    # 검출기를 갈아끼우거나 bbox 를 직접 주고 싶을 때 이 둘을 따로 호출한다.
    ...
    def detect_hands(
        ...
        Returns
        -------
        boxes    : (N, 4) float32  [x1, y1, x2, y2] 픽셀
        is_right : (N,)   bool     True = 오른손
        scores   : (N,)   float32  검출 확신도

        다른 검출기로 바꾸려면 이 함수를 쓰지 말고 같은 형식의 (boxes, is_right, scores)
        를 만들어 reconstruct() 에 넘기면 된다.
        """
        return self._detect_hands(img_bgr)
    ...
    def reconstruct(
        ...
        """
        2단계 — bbox 마다 크롭 → WiLoR 추론 → MANO 파라미터·메시·관절·cam_t.

        Parameters
        ----------
        img_bgr  : (H, W, 3) uint8, OpenCV BGR (detect_hands 에 준 것과 같은 이미지)
        boxes    : (N, 4) float32  [x1, y1, x2, y2] 픽셀
        is_right : (N,)   bool
        scores   : (N,)   float32, 생략 시 1.0 으로 채움 (HandPrediction.score 에만 실림)
        backend  : 생성자 backend 를 이 호출에서만 덮어쓰기

        Returns
        -------
        List[HandPrediction] — boxes 순서대로 하나씩. 빈 boxes 면 빈 리스트.
        """
        backend = backend or self.backend
        boxes    = np.asarray(boxes, dtype=np.float32).reshape(-1, 4)
        is_right = np.asarray(is_right, dtype=bool).reshape(-1)
        if scores is None:
            scores = np.ones(len(boxes), dtype=np.float32)
        scores = np.asarray(scores, dtype=np.float32).reshape(-1)
        if not (len(boxes) == len(is_right) == len(scores)):
            raise ValueError(
                f"boxes/is_right/scores 길이 불일치: {len(boxes)}/{len(is_right)}/{len(scores)}"
            )
        if len(boxes) == 0:
            return []
    ...
    def mano_faces(self) -> np.ndarray:
        """
        MANO 메시 면 인덱스 (1538, 3) int — 오른손 기준.
        HandPrediction.vertices(778, 3) 와 짝지어 trimesh 등으로 .obj 를 만들 때 사용.
        왼손은 정점 x 가 반전돼 있으므로 faces[:, [0, 2, 1]] 로 면 방향을 뒤집어야 한다
        (WiLoR renderer.faces_left 관례). pyrender 없이 쓸 수 있다.
        """
        if self._wilor_model is None:
            raise RuntimeError("mano_faces 는 WiLoR 백엔드가 로드돼야 한다 (backend='wilor').")
        return np.asarray(self._wilor_model.mano.faces, dtype=np.int64)
...
def _kp2d_crop_to_full(
    kp2d_norm: np.ndarray,   # (21, 2) in [-0.5, 0.5] — 크롭 한 변이 [-0.5, 0.5]
    box_center: np.ndarray,  # (2,)
    box_size: float,
    model_size: int = 256,
) -> np.ndarray:             # (21, 2) pixel coords
    # [MimicForge] 원본은 [-1,1] 정규화를 가정해 스케일이 절반이 되는 버그가 있었다.
    # WiLoR 의 pred_keypoints_2d 는 perspective_projection(focal/IMAGE_SIZE) 산출이라
    # 크롭 폭 전체가 [-0.5, 0.5] (4DHumans/HaMeR 계열 관례).
    kp = (kp2d_norm + 0.5) * box_size
